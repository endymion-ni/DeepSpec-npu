...
DEFAULT_MODEL = "dsv4"
DEFAULT_SERVER_ADDRESS = ["127.0.0.1:8900"]
DEFAULT_CONCURRENCY = 16


def parse_args():
    parser = argparse.ArgumentParser(
        description="Regenerate JSONL conversations through a DeepSeek V4 Flash vLLM server."
    )
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument("--server-address", nargs="+", default=DEFAULT_SERVER_ADDRESS)
    ...
    if not args.enable_thinking and not args.disable_thinking:
...
def call_server(args, server_address, sample, max_tokens=None):
...
def validate_server(args, server_address, probe):
    ...
    try:
        result = call_server(args, server_address, dict(probe), max_tokens=1)
    except Exception as exc:
...
def validate_servers(args):
    ...
    print(f"Validating {server_count} server(s) in parallel...", flush=True)
...
def print_config(args):
    thinking_mode = "enabled" if args.enable_thinking else "disabled"
    print("Configuration:")
...
def main():
    ...
    with (
        ...
        for line in input_handle:
            ...
            future = executor.submit(call_server, args, server_address, sample)
